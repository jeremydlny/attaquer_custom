...
const BatteryStatus: Component<BatteryStatusProps> = (props) => {
  ...
  const BatteryTime = () => {
    ...
    if (props.battery?.state === "charging") {
      const timeTillFull = props.battery?.timeTillFull ?? 0;
      const hours = Math.trunc(timeTillFull / 3600000);
      if (hours) {
        result += "Charging: " + hours + "h ";
      }
      result +=
        Math.trunc((timeTillFull % 3600000) / 60000) +
        "min left";
    } else if (props.battery?.state === "discharging") {
      const timeTillEmpty = props.battery?.timeTillEmpty ?? 0;
      const hours = Math.trunc(timeTillEmpty / 3600000);
      if (hours) {
        result += "Discharging: " + hours + "h ";
      }
      result +=
        Math.trunc((timeTillEmpty % 3600000) / 60000) +
        "min left";
    } else {
      result += "idle";
    }
